...
*   **Regra de Ativação:** O Claude Code só tem permissão para operar e executar no terminal após o usuário conceder autorização explícita via chat (ex: *"pode executar no Claude"*).
...
*   *Fonte única da verdade de planos:* `src/lib/plans.ts`
...
*   **Fallbacks por Ambiente:** Mecanismo robusto via `getShoppingFallback` em `src/lib/room-products.ts` para garantir que o fluxo de compras não quebre mesmo offline ou sem produtos cadastrados.
